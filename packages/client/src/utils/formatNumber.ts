...
const numberFormatter = (options?: Intl.NumberFormatOptions): NumberFormatter => {
 ...
 * See __tests__/formatNumber.test.ts for full usage and behavior.
 */
export const significantDigitsNumberFormatter = (significantDigits: number): NumberFormatter =>
  numberFormatter({
...
export const getThousandsSeparator: SeparatorGetter = _locale => {
  return new Intl.NumberFormat(_locale).formatToParts(10000.5).find(part => part.type === 'group')?.value!;
};

export const getDecimalSeparator: SeparatorGetter = _locale => {
  return new Intl.NumberFormat(_locale).formatToParts(10000.5).find(part => part.type === 'decimal')?.value!;
};
